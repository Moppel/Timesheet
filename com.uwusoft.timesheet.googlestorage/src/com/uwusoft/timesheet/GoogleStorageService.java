...
import com.uwusoft.timesheet.StorageService;
...
public class GoogleStorageService implements StorageService {
    ...
    public GoogleStorageService() {
        try {
            ...
            props.load(this.getClass().getResourceAsStream("google.properties"));
    ...
    public void storeTimeEntry(Date dateTime, String task) throws IOException, ServiceException {
        storeTimeEntry(dateTime, task, null);
    }

    public void storeTimeEntry(Date dateTime, String task, String defaultTotal) throws IOException, ServiceException {
        ...
        if (defaultTotal == null) timeEntry.getCustomElements().setValueLocal(TIME, new SimpleDateFormat(timeFormat).format(dateTime));
        if (CHECK_IN.equals(task)) timeEntry.getCustomElements().setValueLocal(TASK, task);
        else {
            if (defaultTotal != null) timeEntry.getCustomElements().setValueLocal(TOTAL, defaultTotal);
            taskLink = taskLinkMap.get(task);
        }
        service.insert(listFeedUrl, timeEntry);

        reloadWorksheets();
        if (taskLink != null) createUpdateCellEntry(defaultWorksheet, defaultWorksheet.getRowCount(), 7, "=" + taskLink); // todo column 7 for task
    ...
    public void storeLastWeekTotal() {
        try {
            ...
            reloadWorksheets();
            createUpdateCellEntry(defaultWorksheet, defaultWorksheet.getRowCount(), 5, "=SUM(R[-1]C[-1]:R[-" + (rowsOfWeek - 1) + "]C[-1])"); // todo column WT=5
        } catch (IOException e) {
            e.printStackTrace();  //Todo
        } catch (ServiceException e) {
            e.printStackTrace();  //Todo
    ...
    public void submitEntries() throws IOException, ServiceException, ParseException {
        reloadWorksheets();
        ListFeed feed = service.getFeed(listFeedUrl, ListFeed.class);
        List<ListEntry> listEntries = feed.getEntries();
        ...
        for (; i > 0; i--) {
            ...
            entry.addSubmitEntry(task, Double.valueOf(elements.getValue(TOTAL)), new SubmissionServiceImpl());
        }
    }

    private void loadTasks() throws IOException, ServiceException {
    ...
    private void createUpdateCellEntry(WorksheetEntry worksheet, int row, int col, String value) throws ServiceException, IOException {
